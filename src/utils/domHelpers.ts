 ...
 */
export function extractPostId(element: Element): string {
  ...
  const postLink = element.querySelector<HTMLAnchorElement>('a[href*="/feed/update/"], a[href*="/posts/"]');
  if (postLink?.href) {
 ...
 */
export function extractAuthorName(element: Element): string {
  ...
  if (actorContainer) {
    ...
    const authorLink = actorContainer.querySelector<HTMLAnchorElement>('a[href*="/in/"]');
  ...
  const links = element.querySelectorAll<HTMLAnchorElement>('a[href*="/in/"]');
 ...
 */
export function extractAuthorProfile(element: Element): string {
  const actorContainer = element.querySelector('.feed-shared-actor, .update-components-actor');
  const searchRoot = actorContainer || element;
  const link = searchRoot.querySelector<HTMLAnchorElement>('a[href*="/in/"]');
 ...
 */
export function extractPostContent(element: Element): string {
 ...
 */
export function extractPostUrl(element: Element): string {
  ...
  const links = element.querySelectorAll<HTMLAnchorElement>('a[href*="/feed/update/"], a[href*="/posts/"]');
  for (const link of links) {
 ...
 */
export function findPostCard(element: Element): Element | null {
  let current: Element | null = element;
  let urnMatch: Element | null = null;
  while (current) {
